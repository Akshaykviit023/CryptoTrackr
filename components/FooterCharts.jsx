...
const FooterCharts = ({ trendingCoinsProps }) => {
    ...
    const [trendingNft, setTrendingNft] = useState(null);
  ...
  return (
    <div className="p-12 bg-white ">
      ...
      <CarouselContent className="-ml-1">
        {trendingNft && trendingNft.map((nft, index) => (
            ...
            <div className="p-1">
              <Card className="py-4">
                ...
                <Image 
                        ...
                        <p className="text-base font-medium">{nft.name}</p>
                        <div className={`px-2 py-1 rounded-md ${nft.floor_price_24h_percentage_change.toFixed(2) > 0 ? "bg-green-50" : "bg-red-50"}`}>
                        <p className={`text-xs font-medium  flex items-center gap-1 ${nft.floor_price_24h_percentage_change.toFixed(2) > 0 ? "text-green-500" : "text-red-500"}`}>
                            <BiSolidUpArrow className={`text-xs ${nft.floor_price_24h_percentage_change.toFixed(2) > 0 ? "" : "rotate-180" }`}/>
                            {nft.floor_price_24h_percentage_change.toFixed(2)}%</p>
                        </div>
                        </div>
                        <p className="text-xl font-medium">{nft.data.floor_price}</p>
                        <Image 
