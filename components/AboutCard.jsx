...
const AboutCard = ({ performanceMetrics, currentCoin }) => {
  ...
  return (
    <div className="mt-6 bg-background rounded-xl p-2 md:p-6">
        ...
        { performanceData && (
            <div>
        <h1 className="text-2xl font-semibold">About {performanceData.name}</h1>
        <h2 className="text-lg font-semibold">What is {performanceData.name}?</h2>
        
        
        <p className="text-[15px] font-medium text-[#3E424A]">{performanceData.name}&apos;s price today is US{formatNumber(performanceData.market_data.current_price.usd, 'USD')}, with a 24-hour trading volume of {formatNumber(performanceData.market_data.total_volume.usd, 'USD')}. {performanceData.symbol} is {performanceData.market_data.price_change_percentage_24h_in_currency.usd.toFixed(2)}% in the last 24 hours. It is currently -7.70% from its 7-day all-time high of $18,366.66, and 3.40% from its 7-day all-time low of $16,394.75. {performanceData.symbol} has a circulating supply of 19.24 M {performanceData.symbol} and a max supply of 21 M {performanceData.symbol}.</p>
        ...
        <Separator className="my-4"/>
        <h2 className="text-lg font-semibold">Already Holding {performanceData.name}?</h2>
        </div>
        )
}
